---
title: "EAH 2024 - Módulo discapacidad"
...
diseno <- readxl::read_excel(here(temp_dir,'eah2024_bu_ampliada_ind_diseño_de_registros.xlsx'))
calculo_cv <- readxl::read_excel(here(temp_dir,'eah2024_bu_ampliada_calculo_cv.xls'))

base <- 
  read_delim(here(temp_dir,'eah2024_bu_ampliada_ind.txt'),
             delim = ';')
...
# extraer valores
prop <- coef(res)["I(dd_con_dif == 1)TRUE"]
se   <- SE(res)["I(dd_con_dif == 1)TRUE"]

ci   <- confint(res)
low  <- ci["I(dd_con_dif == 1)TRUE", 1]
upp  <- ci["I(dd_con_dif == 1)TRUE", 2]

# formato texto
prop_text <- percent(prop, accuracy = 0.1)
low_text  <- percent(low,  accuracy = 0.1)
upp_text  <- percent(upp,  accuracy = 0.1)

```

## Prevalencia de dificultad

La prevalencia de población con alguna dificultad es de 
**`r prop_text`** 
(IC 95%: `r low_text`–`r upp_text`).
