...
Porcentaje de personas con dificultades que no pueden valerse por sí mismos (dd9)

```{r , echo=FALSE}

base_survey <- 
  base_survey %>%
  mutate(dificultad_cuidados = case_when(dd9 > 2 ~ 'si',
                                         TRUE ~ 'no'))
  
  
base_survey %>% 
  filter(alguna_dificultad == 'si' ) %>% 
  group_by(dificultad_cuidados ) %>% 
  summarise(porcentaje = round(survey_prop( level = 0.95, 
                               vartype = 'cv')*100),
            Total = survey_total(level = 0.95,
                                 vartype = 'cv' )) %>% 
...
Personas con dificultad para valerse por si mismas sobre el total poblacional.
